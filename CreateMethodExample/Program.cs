        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                using (var assembly = AssemblyDefinition.ReadAssembly(inputAssemblyFileName, new ReaderParameters() { ReadSymbols = true, ReadWrite = false }))
                {
                    ...
                    var method = new MethodDefinition("Test", MethodAttributes.Private | MethodAttributes.Static, assembly.MainModule.TypeSystem.Void);
                    ...
                    instructions.Insert(1, Instruction.Create(OpCodes.Call, module.ImportReference(typeof(Console).GetMethod("WriteLine", new Type[] { typeof(string) }))));
